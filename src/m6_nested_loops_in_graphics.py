...
def draw_wall_on_right(rectangle, n, window):
    ...
    upper = rg.Point(upper_right.x, upper_right.y)
    lower = rg.Point(lower_left.x, lower_left.y)

    for k in range(n):
        for j in range(k + 1):
            new_rectangle = rg.Rectangle(upper, lower)
            new_rectangle.attach_to(window)
            window.render(0.1)
            upper.x = upper.x - width
            lower.x = lower.x - width
        lower.x = upper_right.x
        lower.y = lower.y + height
        upper.x = lower_left.x
        upper.y = upper.y + height
